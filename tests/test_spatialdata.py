...
def test_sdatawidget_images(make_napari_viewer: Any):
    ...
    assert len(widget._viewer.viewer.layers) == 0
    ...
    assert len(widget._viewer.viewer.layers) == 1
    assert isinstance(widget._viewer.viewer.layers[0], Image)
    assert widget._viewer.viewer.layers[0].name == list(sdata.images.keys())[0]
    sdata.images["image"] = to_multiscale(sdata.images["blobs_image"], [2, 4])
    widget.elements_widget._onClickChange("global")
    widget._onClick("image")
    assert len(widget._viewer.viewer.layers) == 2
    assert (widget._viewer.viewer.layers[0].data == widget._viewer.viewer.layers[1].data).all()
...
def test_sdatawidget_labels(make_napari_viewer: Any):
    ...
    assert len(widget._viewer.viewer.layers) == 1
    assert widget._viewer.viewer.layers[0].name == list(sdata.labels.keys())[0]
    assert isinstance(widget._viewer.viewer.layers[0], Labels)
    assert isinstance(widget._viewer.viewer.layers[0].metadata.get("adata"), AnnData)
    assert (
        widget._viewer.viewer.layers[0].metadata.get("adata").n_obs
        == (sdata.table.obs[sdata.table.uns["spatialdata_attrs"]["region_key"]] == list(sdata.labels.keys())[0]).sum()
    )
    assert widget._viewer.viewer.layers[0].metadata.get("labels_key") is not None
...
def test_sdatawidget_points(caplog, make_napari_viewer: Any):
    ...
    assert len(widget._viewer.viewer.layers) == 1
    assert widget._viewer.viewer.layers[0].name == list(sdata.points.keys())[0]
    assert isinstance(widget._viewer.viewer.layers[0], Points)
    assert isinstance(widget._viewer.viewer.layers[0].metadata.get("adata"), AnnData)
    assert widget._viewer.viewer.layers[0].metadata.get("adata").n_obs == len(sdata.points["blobs_points"]["x"])
    assert (
        len(widget._viewer.viewer.layers[0].metadata.get("adata").obs.keys()) == sdata.points["blobs_points"].shape[1]
    )
    ...
    assert widget._viewer.viewer.layers[1].metadata.get("adata").n_obs == 100000
...
def test_layer_visibility(qtbot, make_napari_viewer: Any):
    ...
    points = viewer.viewer.layers[0]
    labels = viewer.viewer.layers[1]
